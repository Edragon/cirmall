...
Switch::~Switch(){/*nothing to destroy*/}
...
void Switch::startWebServer(){
  ...
  server->begin();
  Serial.println("WebServer started on port: ");  Serial.println(localPort);
}
...
void Switch::respondToSearch(IPAddress& senderIP, unsigned int senderPort) {
  Serial.print("\nSending response to ");  	Serial.println(senderIP);
  Serial.print("Port : ");				 	Serial.println(senderPort);
...
void Switch::handleSetupXml(){
  ...
  String setup_xml = "<?xml version=\"1.0\"?>"
    ...
    Serial.print("Sending :");    Serial.println(setup_xml);
}
